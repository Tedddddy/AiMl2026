...
\begin{frame}{Proof steps}
	\begin{itemize}
        ...
        \item Keynote: For any Kripke frame $F = (W, R_1,...R_n)$, we can construct a neighborhood frame $N(F)$, i.e.
...
\begin{frame}[t]{Surjective bounded morphism}
	...
	Let $N_\omega^r \times_n^+ N_\omega^r = (X \times X, \tau_1, \tau_2, \tau)$ and $N(T_{\omega,\omega,\omega}^{r,m}) = ((\mathbb{N} \times \{0,1,2\} \cup \{\star\})^*, \sigma_1, \sigma_2, \sigma)$\pause
	\begin{theorem}
	 There is a surjective bounded morphism $g: N_\omega^r \times_n^+ N_\omega^r \to N(T_{\omega,\omega,\omega}^{r,m})$. 
	\end{theorem}
	Note: $Log(N_w^r) = T$ and hence $Log(N_w^r \times_n^+ N_w^r) \in T \times_n^+ T$ 
...
\begin{frame}[t]{Surjective bounded morphism}
	...
	Proof sketch: We fix a bijection $h: \mathbb{N} \to \mathbb{N} \times \mathbb{N}$. \pause
			\[
				g: X \times X \to (\mathbb{N} \times \{0,1,2\} \cup \{\star\})^* 
		\] \pause
\vspace{-0.4cm}
			\[
\begin{array}{cccccc}
\alpha &= \star & 1 & 2 & \star & \star^\omega\\
\beta  &= 3     & \star & 7 & \star & \star^\omega\\
\hline \pause
g'(\alpha,\beta) &= (3,2) & (1,0) & (h(7,2),0) & \star & \star^\omega
\end{array}
\]
 \pause


Then $g(\alpha,\beta) = (3,2)(1,0) h((7,2),0)$
...
\hline 
g'(\alpha,\beta) &= (3,2) & (1,0) & (h(7,2),0) & \star & \textcolor{red}{(h(6,7),0)}& \star^\omega
\end{array}
\]
\vspace{0.5cm}
