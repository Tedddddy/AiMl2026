...
\begin{frame}{Log$(T_{\omega,\omega,\omega}^{r,m}) = T_3m$}
    \begin{itemize}
        \item Pick $\mathbb{C} = \{F \mid F \vDash T_3m\}$
    \end{itemize}
    \begin{Lemma}
        $Log(\mathbb{C}) = T_3m$
    \end{Lemma}
    \begin{itemize}
        \item By Sahlqvist Theorem
    \end{itemize} \pause

    \begin{Lemma}
        $T_3m$ has the finite model property
    \end{Lemma}
    
    \begin{itemize}
        \item By Filtration Theorem
    \end{itemize}
\end{frame}
...
\begin{itemize}
    \item Note: $\Box p \to \Box_1 p \land \Box_2$ is derivable
    \item Log($T_{\omega,\omega,\omega}^{r,m}) \subseteq T_3m$ shown by unraveling
\end{itemize}
\end{frame}
...
\begin{frame}{Surjective bounded morphism}
	\begin{theorem}
		Let $N_\omega^r \times_n^+ N_\omega^r = (X \times X, \tau_1, \tau_2, \tau)$ and $N(T_{\omega,\omega,\omega}^{r,m}) = ((\mathbb{N} \times \{0,1,2\} \cup \{\star\})^*, \sigma_1, \sigma_2, \sigma)$ be two 
		full product frames. Then there is a surjective bounded morphism $g: N_\omega^r \times_n^+ N_\omega^r \to N(T_{\omega,\omega,\omega}^{r,m})$. Note: $Log(N_w^r) = T$ and hence $N_w^r \times_n^+ N_w^r \in T \times_n^+ T$
	\end{theorem} \pause

		Proof sketch: We fix a bijection $h: \mathbb{N} \to \mathbb{N} \times \mathbb{N}$. Let $a = a_0a_1...$ and $b = b_0b_1... \in X$ We define

			
        %f(a, b) =
       % \begin{cases}
       % \star, & \text{if }a=b=\star,\\
       % (a,1), & \text{if } b = \star, a\in \mathbb{N}, \\
       % (b,2), & \text{if } a = \star, b\in \mathbb{N}, \\
       % (h(a, b),0), & \text{otherwise}.
    	%\end{cases}

		\[
				g: X \times X \to (\mathbb{N} \times \{0,1,2\} \cup \{\star\})^* \mbox{ with } g(a,b) = forg(f(a_0b_0)f(a_1b_1)f(a_2b_2)...) 
		\]
		\[
      		  	f(a, b) =
       			 \begin{cases}
...
\begin{frame}{Surjective bounded morphism}
	...
	\begin{itemize}
		...
		\item $R(g(\alpha,\beta)) \subseteq g(U_m(\alpha) \times U_m(\beta)) \subseteq g(U)$ are not hard to see
	\end{itemize}
		\vspace{1cm}
	    {\transparent{0.3}
			Note: $\tau(x,y) = \{U \mid \exists k,j \in \mathbb{N}: U_k(x) \times U_j(y) \subseteq U\}, \sigma(x) = \{U \mid R(x) \subseteq U\}$.

   		2nd cond: for all $1 \le i \le k$, for all $w \in W$ and all $U \in \tau_i(w)$, there is a $V\in \sigma_i(f(w))$ with $V = f(U)$ 
...
\begin{frame}{Surjective bounded morphism}
		...
		{\transparent{0.3}
		Note: $\tau(x,y) = \{U \mid \exists k,j \in \mathbb{N}: U_k(x) \times U_j(y) \subseteq U\}, \sigma(x) = \{U \mid R(x) \subseteq U\}$\newline
		3rd cond: for all $1 \le i \le k$, for all $w \in W$ and all $V \in \sigma_i(f(w))$, there exists $U \in \tau_i(w)$ such that $f(U) \subseteq V$ 
		}
\end{frame}
...
\begin{frame}{Surjective bounded morphism}
		\begin{theorem}
			...
			Remark: if $m \ngtr max\{st(\alpha),st(\beta)\}$ then the subset relation fails. Example: Let $\alpha = 12\star^\omega, \beta = \star23\star^\omega$
			and $m = 1$. Then $g(\alpha, \beta) = (1,1)h(2,2)(3,2)$. Assume $\alpha' = 123\star^\omega$. Hence,
			$g(\alpha', \beta) = (1,1),h(2,2)h(3,3) \notin R(g(\alpha,\beta))$
			\vspace{0.5cm}
\end{frame}