...
\begin{frame}{Proof steps}
	...
	\item Idea: create an infinite branching and depth Kripke tree with three relations (called $T_{\omega,\omega,\omega}^{r,m}$) with $Log(T_{\omega,\omega,\omega}^{r,m}) = T_3m$
    \item Note: For any Kripke frame $F = (W, R_1,...R_n)$, we can construct a neighborhood frame $F'$, i.e.
		for any $w \in W$: $\tau_i(w) = \{U \mid R_i(w) \subseteq U \subseteq W\}$ where $Log(F) = Log(F')$ 
			\end{itemize}
\vspace{1.2cm}
\[ 
	{\transparent{0.3} T \otimes T \otimes T + (\Box p \to \Box_1\Box_2 p \land \Box_2\Box_1 p) = T_3m }
...
\begin{frame}{Definition of the neighborhood frame $N_w^r$}
    \begin{definition}
		...
		\]
		where \emph{forg}($\alpha$) deletes all $\star$ and $ls(\alpha) = min\{N \mid \forall k \geq N: a_k = \star\}$
...
\begin{frame}{Example of $U_k(\alpha)$}
	...
	Let $\alpha = \star \star 12 \star 3 \star 4 \star \star^\omega \in X$. Then $ls(\alpha) = 9$. Let's consider $U_9(\alpha)$.
	Then 
	\[
...
\begin{frame}{Surjective bounded morphism}
	...
	\end{theorem} \pause
		...
		Proof sketch: We fix a bijection $h: \mathbb{N} \to \mathbb{N} \times \mathbb{N}$. \pause 
		Let $\alpha = a_0a_1...$ and $\beta = b_0b_1... \in X$ We define
       ...
       % (h(a, b),0), & \text{otherwise}.
		...
		\[
				g: X \times X \to (\mathbb{N} \times \{0,1,2\} \cup \{\star\})^* \mbox{ with } g(\alpha,\beta) = forg(f(a_0b_0)f(a_1b_1)f(a_2b_2)...) 
		\]
		\[
      		  	f(a, b) =
       			 \begin{cases}
